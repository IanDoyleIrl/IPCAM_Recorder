 ...
 * frame boundary.  The input stream can begin mid-frame and this parser will still find the first
 * complete MJPEG data frame.
 *
 * Axis camera JPEG images are 4:2:2

 * @see http://www2.axis.com/files/developer/camera/JPEG_format_1_1.pdf JPEG_format_1_1
 * @author Jason Thrasher
 */
public class MjpegFrameParser implements Runnable {
    private static Logger mLog = Logger.getLogger(MjpegFrameParser.class); //logging mechanism
    protected EventListenerList mListeners = new EventListenerList(); //event listeners for parsing events
    ...
    /**
     * Create a new frame parser with the given input stream, and given buffer size.
     *
     ...
     *
     * @param mjpegStream from any datasource.
     */
    ...
    /**
     * Add new parse listeners.  This is how other code gets notification of new MJPEG frames.
     * @param listener
     */
    ...
    protected void notifyListeners(MjpegParserEvent event) {
        ...
        for (int i = 0; i < listeners.length; i += 2) {
            if (listeners[i] == MjpegParserListener.class) {
                ((MjpegParserListener) listeners[i + 1]).onMjpegParserEvent(event);
            }
        }
    }

    /**
     * Start the parser thread in a blocking fashion.
     * Parser will notify all listeners of MJPEG parse events.
     * This call will block until the end of the stream is reached.
     * To start asynchronously, use the start() method.
     */
    public void parse() {
